...
func newRouter(cfg *config.Config, db *gorm.DB) *gin.Engine {
	...
	employeeHandler := handlers.NewEmployeeHandler(db)             // FactoryFlow Employee CRUD (FRESH-04 read, FRESH-05 write) — เป็นแหล่งข้อมูลบุคลากรเดียวของระบบแล้ว (แทน personnel เดิมที่ถูกลบออก)
	...
	api.Use(middleware.Auth(secret))
	{
		...
		// บุคลากร FactoryFlow (Employee) — auth = Friend middleware.Auth (no role policy yet)
		//   GET (FRESH-04) · POST/PUT/DELETE + 1:1 UserAccount provisioning (FRESH-05)
		//   GET :id/account = safe read of the linked login account (FRESH-07)
		api.GET("/employees", employeeHandler.ListEmployees)
		api.GET("/employees/:id", employeeHandler.GetEmployee)
		api.GET("/employees/:id/account", employeeHandler.GetEmployeeAccount)
		...
		// FRESH-12 — 1:1 login-account controls for the ported Personnel edit dialog
		api.PATCH("/employees/:id/account", employeeHandler.SetAccountActive)
		api.POST("/employees/:id/account/reset-password", employeeHandler.ResetAccountPassword)

		// งาน FactoryFlow (Task) + การมอบหมายพนักงาน (Task Assignment) — auth = Friend middleware.Auth (FRESH-08)
		api.GET("/tasks", taskHandler.ListTasks)
		api.GET("/tasks/:id", taskHandler.GetTask)
		api.POST("/tasks", taskHandler.CreateTask)
		api.PUT("/tasks/:id", taskHandler.UpdateTask)
		api.DELETE("/tasks/:id", taskHandler.DeleteTask)
		api.GET("/tasks/:id/assignments", taskAssignmentHandler.ListAssignments)
		api.POST("/tasks/:id/assignments", taskAssignmentHandler.CreateAssignment)
		api.DELETE("/tasks/:id/assignments/:assignmentId", taskAssignmentHandler.DeleteAssignment)
		// FRESH-12 — every assignment in one call, for the ported shift/assignment board
		api.GET("/tasks/assignments", taskAssignmentHandler.ListAllAssignments)
		...
		api.GET("/production/finished-goods", productionHandler.ListFinishedGoods)
		api.DELETE("/production/finished-goods/:id", productionHandler.DeleteFinishedGood)
