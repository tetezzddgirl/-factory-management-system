...
// seedSummary is the post-seed state read back from the database (SELECT only).
type seedSummary struct {
	Employees          int64
	Accounts           int64
	ActiveAccounts     int64
	EmployeesWithAcct  int64 // distinct employees that have >=1 account
	EmployeesMultiAcct int64 // employees with >1 account (must be 0)
	PerDeptEmployees   map[string]int64
	PerDeptAccounts    map[string]int64 // accounts reached via the employee FK
...
// verify runs read-only aggregate queries over the FactoryFlow tables.
func verify(db *gorm.DB) (seedSummary, error) {
	s := seedSummary{
		PerDeptEmployees: map[string]int64{},
	...
	// tasks / task_assignments must be UNTOUCHED by this seed
...
// ok reports whether the summary matches the FRESH-10.6C blueprint exactly.
...
func printSummary(s seedSummary) {
	...
	fmt.Printf("  [info] tasks=%d task_assignments=%d (must be unchanged by this seed)\n", s.Tasks, s.TaskAssignments)
}
