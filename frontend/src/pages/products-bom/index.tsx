          ...
          {boms.map((bom) => (
            <Grid item xs={12} md={6} key={bom.id}>
              ...
              >
                ...
                {/* Materials List */}
                <Typography variant="caption" fontWeight={600} color="text.secondary" sx={{ display: "block", mt: 2.5, mb: 1 }}>
                ...
                {bom.steps && bom.steps.length > 0 ? (
                  <>
                    <Typography variant="caption" fontWeight={600} color="text.secondary" sx={{ display: "block", mt: 2, mb: 0.5 }}>
                      ขั้นตอนการผลิต:
                    </Typography>
                    <Box component="ol" sx={{ pl: 2.5, m: 0, fontSize: "0.8125rem", color: "text.secondary" }}>
                      {bom.steps.map((step, idx) => (
                        <li key={`${bom.id}-step-${idx}`}>
                          <Typography variant="body2" fontSize="0.8125rem" color="text.primary">
                            {step}
                          </Typography>
                        </li>
                      ))}
