...
import {
  maintenanceApi, machinesApi, personnelApi,
  type ApiMachine, type ApiMaintenanceLog, type ApiMaintenanceOrder, type ApiPersonnel,
} from "@/lib/api-client";
import {
...
function MaintenancePage() {
  ...
  const [technicians, setTechnicians] = useState<ApiPersonnel[]>([]);
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        ...
        <Stack spacing={2}>
          ...
          >
            {technicians.map((t) => (
              <MenuItem key={t.id} value={t.name}>{t.name} — {t.role}</MenuItem>
            ))}
