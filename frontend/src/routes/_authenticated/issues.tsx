...
function ProblemPage() {
  ...
  async function loadIssues() {
    ...
    setError(null);
    try {
      const [data, orders, staff] = await Promise.all([issuesApi.list(), workOrdersApi.list(), employeesApi.list()]);
      setProblem(data ?? []);
  ...
  ? employeeOptions(personnel)
  : [];
  const currentUserEmail = getSession()?.email ?? "";
  const currentReporter = personnel.find(
    (p) => p.email?.toLowerCase() === currentUserEmail.toLowerCase());
  const currentReporterName = currentReporter
    ? `${currentReporter.employeeId} — ${employeeFullName(currentReporter)}` : "";
  const currentPlanner = personnel.find(
    (p) => p.email?.toLowerCase() === currentUserEmail.toLowerCase());
  const currentPlannerName = currentPlanner
    ? `${currentPlanner.employeeId} — ${employeeFullName(currentPlanner)}` : "";
  ...
  function openDetail(iss: ApiIssue) {
    ...
    // ถ้ายังไม่มีใครลงชื่อรับผิดชอบไว้ ให้เติมบัญชีที่ล็อกอินอยู่ตอนนี้ให้อัตโนมัติ (แก้ไขเองได้ถ้าไม่ใช่คนนี้)
    setSolutionProviderID(
    iss.solution_provider_id
    ? getPersonnelLabel(iss.solution_provider_id) : currentPlannerName);
  ...
  async function saveSolution() {
    ...
    try {
      await issuesApi.update(selected.issue_id, {
        solution_provider_id: solutionProviderID.split(" — ")[0],
        solutions: solution,
        status,
      });
...
function getPersonnelLabel(id: string) {
  const person = personnel.find((p) => p.employeeId === id);

  return person
    ? `${person.employeeId} — ${employeeFullName(person)}`
    : id;
}
      ...
      actions={
        <AddItemDialog
          ...
          fields={[
            ...
            { name: "reporterID", label: "เจ้าหน้าที่ผู้แจ้งปัญหา", type: "select", options: personnelOptions, placeholder: "PSN-001 สมชาย ใจดี", defaultValue: currentReporterName }, // เติมจากบัญชีที่ล็อกอินอยู่ให้อัตโนมัติ
