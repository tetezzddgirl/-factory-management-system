...
/**
 * Backend API Base URL (Go Gin Server on localhost:8090)
 */
const API_HOST = import.meta.env.VITE_API_URL || "http://localhost:8090";
...
export async function getPendingFGTransfers(): Promise<PendingFGItem[]> {
  try {
    const token = localStorage.getItem("ff:token") || localStorage.getItem("token") || "";
    const res = await fetch("http://localhost:8090/api/production/transfers/pending-fg", {
      headers: { Authorization: `Bearer ${token}` },
    });
    if (!res.ok) return [];
    return await res.json();
...
export async function receiveTransferApi(transferID: string, receivedBy = "เจ้าหน้าที่ฝ่ายคลังสินค้า"): Promise<boolean> {
  try {
    const token = localStorage.getItem("ff:token") || localStorage.getItem("token") || "";
    const res = await fetch(`http://localhost:8090/api/production/transfers/${transferID}/receive`, {
      method: "PATCH",
      headers: {
