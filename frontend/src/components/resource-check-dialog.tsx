    ...
    >
      <Box sx={{ p: 2, borderRadius: 2, background: "rgba(74,144,226,0.06)" }}>
        <Stack direction="row" spacing={1} sx={{ alignItems: "center", mb: 1.5 }}>
          ...
          <Chip size="small" color={color} icon={<Icon sx={{ fontSize: 14 }} />} label={label} />
        </Stack>
        <Stack spacing={1.25}>
          {items.map((it) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      ...
      <DialogContent dividers>
        ...
        <Stack spacing={2}>
          <SectionCard index={0} title="วัตถุดิบ" icon={<Inventory2 color="primary" />} items={data.materials} />
          <SectionCard index={1} title="เครื่องจักร" icon={<PrecisionManufacturing color="primary" />} items={data.machines} />
          <SectionCard index={2} title="บุคลากร" icon={<Groups color="primary" />} items={data.personnel} />
        </Stack>
        <Divider sx={{ my: 2 }} />
