...
function hmrGatePlugin(options = {}) {
	...
	function gateSendTarget(target) {
		...
		target.send = (...args) => {
			...
			if (type === "full-reload") {
				if (Date.now() < startupGraceUntil || initialBuildPending()) return original(...args);
				heldReload = true;
				return;
			}
				...
				srv.middlewares.use((req, res, next) => {
					...
					if (reload) {
						...
						if (typeof restart === "function") {
							const restartDelayMs = Math.max(released > 0 ? drainMs : 0, priorDrainUntil - Date.now(), 0);
							...
							restartTimer.unref?.();
						} else if (wrappedSends.length > 0) wrappedSends[0].original({ type: "full-reload" });
					}
